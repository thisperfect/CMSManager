...
public class ValidationCodeUtils {
        ...
        // 可选字符
        private String codes  = "8FhfE2rvdyaUGBA9exNQ6JMVk34RPZbnHWso7pSDqtucX5jwKzmCTYg";
        // 背景色
        ...
        // 随机生成一个字符
        public char[] randomChar (int count) {
            char[] c = new char[count];
            for (int i = 0; i < count; i++) {
                int index = r.nextInt(codes.length());
                c[i] = codes.charAt(index);
            }
            return c;
        }
        ...
        // 调用这个方法得到验证码
        public BufferedImage getImage (char[] code) {
            BufferedImage image = createImage();//创建图片缓冲区
            Graphics2D g2 = (Graphics2D)image.getGraphics();//得到绘制环境
            // 向图片中画4个字符
            for(int i = 0; i < code.length; i++)  {//循环四次，每次生成一个字符
                String s = StringHelper.toString(code[i]);//随机生成一个字母
