...
public class CachingShiroSessionDao extends CachingSessionDAO {
    private Logger log = LoggerFactory.getLogger(CachingShiroSessionDao.class);
    @Setter
    ...
    protected Serializable doCreate(final Session session) {
        ...
        try {
            ...
            log.debug("sessionId:{}, name:{}, 被创建", sessionId, session.getClass().getName());
        } catch (Exception e) {
    ...
    protected void doUpdate(final Session session) {
        ...
        try {
            if (session instanceof ShiroSession) {
                ...
                log.warn("sessionId:{}, name:{},更新失败", session.getId(), session.getClass().getName());
            }
        } catch (Exception e) {
    ...
    public void unCache(final Serializable sessionId) {
        try {
            ...
            log.warn("删除本地 cache中缓存的Session 失败", e);
        }
    }
